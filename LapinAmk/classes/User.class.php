...
class User
{
  ...
  /**
   * Find the user with the specified Position
   *
    ...
    public static function findByPosition($position)
  {
    try {
      ...
      $stmt = $db->prepare('SELECT * FROM users WHERE position = "staff"');
	 
      $stmt->execute([':position' => $position]);
      $user = $stmt->fetchObject('position');
  ...
  public static function signup($data)
  {
    ...
    if ($user->isValid()) {
      ...
      try {
        ...
        $stmt = $db->prepare('INSERT INTO users (name, email, position, password, activation_token) VALUES (:name, :email, :position, :password, :token)');
    ...
    if ($this->isValid()) {
      ...
      try {
        ...
        if (isset($this->id)) {
          ...
          $sql = 'UPDATE users SET name = :name, email = :email, position = :position, is_active = :is_active, is_admin = :is_admin';
          ...
          $sql = 'INSERT INTO users (name, email, password, is_active, is_admin) VALUES (:name, :email, :position, :password, :is_active, :is_admin)';
        }
  ...
  private function _sendActivationEmail($token)
  {
    ...
    $url = 'http://'.$_SERVER['HTTP_HOST'].'/LapinAmk/activate_account.php?token=' . $token;
